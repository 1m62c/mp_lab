...
with mp_pose.Pose(min_detection_confidence=0.5, min_tracking_confidence=0.5) as pose:
    while cap.isOpened():
        ...
        if mode == 'countdown':
            ...
            if remaining > 0:
                ...
                if results.pose_landmarks:
                    ...
                    if len(landmark_dict[0]) > 24:
                        ex_center_x = (landmark_dict[0][23].x + landmark_dict[0][24].x) / 2
                        ex_center_y = (landmark_dict[0][23].y + landmark_dict[0][24].y) / 2
                    else:
                        ex_center_x, ex_center_y = 0.5, 0.5
        ...
        # === お手本表示（本番） ===
        if mode == 'running' and frame_counter in landmark_dict and offset_initialized:
            overlay = frame.copy()
            landmarks = landmark_dict[frame_counter]
            ...
            for lm in landmarks:
                new_lm = landmark_pb2.NormalizedLandmark()
                new_lm.x = min(max(cx + (lm.x - cx) * SCALE + initial_offset_x, 0.0), 1.0)
                new_lm.y = min(max(cy + (lm.y - cy) * SCALE + OFFSET_Y + initial_offset_y, 0.0), 1.0)
                new_lm.z = (lm.z - cz) * SCALE + cz
                new_lm.visibility = lm.visibility
                example_landmarks.append(new_lm)
